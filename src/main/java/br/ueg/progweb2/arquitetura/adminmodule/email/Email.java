 ...
 * Copyright (c) UEG.
 */
package br.ueg.progweb2.arquitetura.adminmodule.email;
...
public class Email {
	...
	public Email addAllAddressTO(final List<String> addressesTO) {
		if (!CollectionUtils.isEmpty(addressesTO)) {
	...
	public Email addAllAddressBCC(final List<String> addressesBCC) {
		if (!CollectionUtils.isEmpty(addressesBCC)) {
	...
	private void validateParams() throws EmailException {
		...
		if (!CollectionUtils.isEmpty(invalidAddresses)) {
			throw new EmailException("Invalid Recipients: " + String.join(", ", invalidAddresses)
	...
	public void send() throws EmailException {
		try {
			...
			if (!CollectionUtils.isEmpty(addressesCC)) {
